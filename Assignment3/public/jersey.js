...
window.onload = function() {
    ...
    // Check if there are jerseys in the products array
    if (products.some(product => product.jersey !== undefined)) {
        // Loop through jerseys and display them
        for (let i = 0; i < products.length; i++) {
            // Check if the current product is a jersey
            if (products[i].jersey !== undefined) {
                // Display jersey details using the existing HTML structure
                document.querySelector('.row').innerHTML += `
                    <div class="col-md-6 product_card">
                        <div>
                            <h5 style="font-size: 35px; text-align: left; font-family: 'Gill Sans', 'Gill Sans MT', Calibri, 'Trebuchet MS', sans-serif;" class="col-md-10 product_name mb-4">${products[i].jersey}</h5>
                            <h5 style="font-size: 35px; text-align: left; font-family: 'Gill Sans', 'Gill Sans MT', Calibri, 'Trebuchet MS', sans-serif;" class="col-md-10 product_price mb-4">$${(products[i].price2).toFixed(2)}</h5>
                        </div>  

                        <img src="${products[i].image2}" class="img-thumbnail" alt="${products[i].jersey}">
                        ...
                        <div style="height: 90px;">
                            <table style="width: 100%; text-align: center; font-family: 'Gill Sans', 'Gill Sans MT', Calibri, 'Trebuchet MS', sans-serif; font-size: 15px;" id="product_table">
                                <tr>
                                    <!-- Jerseys available for the product -->
                                    <td style="text-align: left; width: 20%;">Jerseys Available: ${products[i].quantityAvailable2}</td>
                                    ...
                                    <td style="text-align: left; width: 20%;">
                                        <input type="text" autocomplete="off" placeholder="Enter Quantity" name="qty${[i]}" id="qty${[i]}_entered" class="inputBox" onkeyup="checkInputTextbox(this,${products[i].quantityAvailable2})" value="0">
                                    </td>
                                </tr>
                                <tr>
                                    <!-- Sold Quantity -->
                                    <td style="text-align: left; width: 35%;" colspan="2">Jerseys Sold: ${products[i].quantitySold2}</td>
                                </tr>
                                <tr>
                                    <!-- Error message -->
