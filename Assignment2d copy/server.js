...
function userIsLoggedIn(request) {
    ...
    if ('email' in request.body && 'password' in request.body) {
        const entered_email = request.body['email'].toLowerCase();
        const entered_password = request.body['password'];

        return user_data[entered_email] && user_data[entered_email].password === entered_password;
    }
...
app.post("/process_login", function (request, response) {
    ...
    if (entered_email.length == 0 && entered_password.length == 0) {
        request.query.loginError = 'Email and Password cannot be empty';
        ...
        request.query.loginError = 'Email does not exist';
    }
